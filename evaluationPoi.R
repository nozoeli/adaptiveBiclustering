...
p_balance <- p_balance + geom_boxplot(outlier.shape = NA, aes(fill = method)) + xlab(bquote(theta * " (multiplication of " * theta['crit'] * ')')) + ylab("Error") + ggtitle(bquote("(M,N," * m^'*' * "," * n^'*' *  ") = (1000,1200,170,140), Poisson")) + theme(plot.title = element_text(hjust = 0.5))
...
p_imbalance <- p_imbalance + geom_boxplot(outlier.shape = NA, aes(fill = method)) + xlab(bquote(theta * " (multiplication of " * theta['crit'] * ')')) + ylab("Error") + ggtitle(bquote("(M,N," * m^'*' * "," * n^'*' *  ") = (4000,500,70,250), Poisson")) + theme(plot.title = element_text(hjust = 0.5))
